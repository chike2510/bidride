...
function ProfileContent() {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem("bidride-theme");
    setDarkMode(stored === "dark");
  }, []);

  useEffect(() => {
    document.documentElement.dataset.theme = darkMode ? "dark" : "light";
    window.localStorage.setItem("bidride-theme", darkMode ? "dark" : "light");
  }, [darkMode]);
  ...
  return (
    <>
      ...
      <main className="flex-1 px-5 sm:px-8 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        {notice && <p className="lg:col-span-2 rounded-input bg-success/10 text-success px-4 py-3 text-sm" role="status">{notice}</p>}
        <Card className="p-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-col gap-1 mt-3 text-sm text-navy/60">
                <span className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => setNotice("Phone verification will be available from account security.")} className="text-gold text-xs font-medium hover:underline">Add</button>
                </span>
                <span className="flex items-center gap-2">
          ...
          <Button variant="secondary" className="w-full mt-5" onClick={() => setNotice("Profile editing will open after identity details are connected.")}>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={() => router.push("/saved-places")} className="text-sm text-gold font-medium hover:underline">Manage</button>
          </div>
          ...
          <button type="button" onClick={() => router.push("/saved-places")} className="w-full flex items-center justify-center gap-1.5 text-sm text-gold font-medium py-3 mt-1 min-h-[44px]">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={() => setNotice("Preference controls will open from the dedicated settings flow.")} className="text-sm text-gold font-medium hover:underline">Manage</button>
          </div>
          <div className="flex flex-col divide-y divide-cardBorder">
            {preferences.map((p) => (
        ...
        <Card className="p-6">
          ...
          <div className="flex flex-col divide-y divide-cardBorder">
            {security.map((s) => (
              <button type="button" key={s.label} onClick={() => setNotice(`${s.label} will open from account security.`)} className="flex items-center gap-3 py-3 w-full text-left min-h-[44px]">
                <span className="h-9 w-9 rounded-full bg-bg flex items-center justify-center shrink-0">
