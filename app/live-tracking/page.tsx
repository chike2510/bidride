...
import { Car, Layers, Locate, MapPin as MapPinIcon, MessageCircle, MessageSquare, Minus, Package, Phone, Plus, Share2 } from "lucide-react";
    ...
    <>
      ...
      <main className="flex-1 flex flex-col">
        <div className="relative flex-1 min-h-[320px] overflow-hidden">
          <Image src="/images/route-placeholder.svg" alt="Live route map" fill className="object-cover" style={{ transform: `scale(${zoom})`, transition: "transform 180ms ease-out" }} />
          <div className="absolute top-4 left-4 flex flex-col gap-2"><button type="button" onClick={() => setNotice("The map is centered on your active route.")} className="h-11 w-11 bg-white rounded-input shadow-soft flex items-center justify-center" aria-label="Center on driver"><Car size={16} /></button><button type="button" onClick={() => setNotice("Map layers are available in the full map integration.")} className="h-11 w-11 bg-white rounded-input shadow-soft flex items-center justify-center" aria-label="Map layers"><Layers size={16} /></button></div>
          <div className="absolute top-4 right-4 flex flex-col gap-2"><button type="button" onClick={() => setZoom((value) => Math.min(1.2, value + 0.05))} className="h-11 w-11 bg-white rounded-input shadow-soft flex items-center justify-center" aria-label="Zoom in"><Plus size={16} /></button><button type="button" onClick={() => setZoom((value) => Math.max(0.9, value - 0.05))} className="h-11 w-11 bg-white rounded-input shadow-soft flex items-center justify-center" aria-label="Zoom out"><Minus size={16} /></button><button type="button" onClick={() => setNotice("The map is centered on your pickup and driver route.")} className="h-11 w-11 bg-white rounded-input shadow-soft flex items-center justify-center" aria-label="Locate route"><Locate size={16} /></button></div>
